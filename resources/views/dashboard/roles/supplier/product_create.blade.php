...
<div class="container flex flex-col bg-white shadow-lg">
    <h3 class=" text-white bg-black p-2 text-center">Add New Product</h3>

    <div class=" w-4/5 mx-auto flex flex-col justify-center items-center">
        

        <form action="{{route('supplier.dashboard.product.create')}}" method="post" enctype="multipart/form-data" class="w-full px-4 py-2 ">
            @csrf
            <div class=" my-4">
            ...
            <div id="title-feild" class=" flex flex-col w-full px-4 py-2 pl-0 ">
                ...
                <input type="text" name="title" id="title" required value="{{old('title')}}" placeholder="e.g. New Brand dell Laptop with 4GB RAM, 250GB hhd" class=" @error('title') !border-red-500 @enderror placeholder:gray-5 w-full rounded-md p-1 pl-4 border-2 border-gray-5 background-gray-1">
                @error('title')
                    <div class="red">{{ $message }}</div>
                @enderror
            </div>
            <!-- end Title field -->
                ...
                <input type="number" name="price" id="price" required value="{{old('price')}}" placeholder="e.g. 30000" class=" @error('price') !border-red-500 @enderror placeholder:gray-5 w-full rounded-md p-1 pl-4 border-2 border-gray-5 background-gray-1">
                @error('price')
                    <div class="red">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div id="type-feild" class=" flex flex-col w-full px-4 py-2 pl-0 ">
                ...
                <input type="text" name="type" id="type" required value="{{old('type')}}" placeholder="e.g. computer" class=" @error('type') !border-red-500 @enderror placeholder:gray-5 w-full rounded-md p-1 pl-4 border-2 border-gray-5 background-gray-1">
                @error('type')
                    <div class="red">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div id="brand-feild" class=" flex flex-col w-full px-4 py-2 pl-0 ">
                ...
                <input type="brand" name="brand" id="brand" required value="{{old('brand')}}" placeholder="e.g. Dell" class=" @error('brand') !border-red-500 @enderror placeholder:gray-5 w-full rounded-md p-1 pl-4 border-2 border-gray-5 background-gray-1">
                @error('brand')
                    <div class="red">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class=" flex flex-col w-full p-4 pl-0 ">
                ...
                <textarea name="description" required id="description" cols="30" rows="10" placeholder="write description of product." class=" w-full rounded-md p-1 pl-4 border-2 border-gray-5 background-gray-1">{{old('description')}}</textarea>
                @error('description')
                    <div class="red">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class=" w-full flex justify-center items-center p-8 space-x-4">
                <button id="login-button" class=" bg-blue-600 max-w-max flex justify-center items-center py-1 px-8 border-2 border-primary rounded-full space-x-2 shadow-md shadow-gray-500 hover:opacity-70">
                    <span class=" white font-bold uppercase">Post</span>
                    <!-- right arrow -->
